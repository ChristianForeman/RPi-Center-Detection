...
#ifndef INC_POLOLU_LIDAR_H_
#define INC_POLOLU_LIDAR_H_

#include "pololu_lidar.h"
#include "stdio.h"
#include "stm32l4xx_hal.h"
#include "main.h"
// BASED ON https://github.com/pololu/vl53l1x-arduino/blob/master/VL53L1X.h

#define LIDAR_I2C_W_ADDR 0x52
#define LIDAR_I2C_R_ADDR 0x53
...
volatile struct RangingData ranging_data;

volatile uint8_t last_status; // status of last I2C transmission
    ...
    void stopContinuous();
    // THE FOLLOWING 4 HAVE BLOCKING DEFAULT INPUTTED TO 1
    uint16_t read(uint8_t blocking);
    uint16_t readRangeContinuousMillimeters(uint8_t blocking); // alias of read()
    uint16_t readSingle(uint8_t blocking);
    uint16_t readRangeSingleMillimeters(uint8_t blocking); // alias of readSingle()
...
// The Arduino two-wire interface uses a 7-bit number for the address,
   ...
   // uint16_t phase_sd0: not used
    ...
    // I2C buses)
    volatile struct ResultBuffer results;

    volatile uint8_t address;

    volatile uint16_t io_timeout;
    volatile uint8_t did_timeout;
    volatile uint16_t timeout_start_ms;

    volatile uint16_t fast_osc_frequency;
    volatile uint16_t osc_calibrate_val;

    volatile uint8_t calibrated;
    volatile uint8_t saved_vhv_init;
    volatile uint8_t saved_vhv_timeout;

    volatile uint8_t distance_mode;

    // Record the current time to check an upcoming timeout against
    //void startTimeout() { timeout_start_ms = millis(); }

    // Check if timeout is enabled (set to nonzero value) and has expired
    // uint8_t checkTimeoutExpired() {return (io_timeout > 0) && ((uint16_t)(millis() - timeout_start_ms) > io_timeout); }
